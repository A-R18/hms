...
const { addPatient, showSinglePatient, showPatients, updatePt, deletePt } = require("../models/patient.model.js")

const registerPatient = async (req, res) => {
    try {
        const enteredPatient = req.body;
        const patientDataMatch = {
            name: enteredPatient.p_name,
        ...
        if (patientSubmitted) {
            ...
            return res.status(400).json("didn't submit patient!");
...
const displayPatients = async (req, res) => {
    try {
        ...
        if(patientsShown){
             return res.status(200).json({message: "patients fetched", ...patientsShown})
        }else
           return res.status(400).json("didn't fetch patients!"); 
...
const displaySinglePatient = async (req, res) => {
  try {
        const patientShown = await showSinglePatient();
        if(patientShown){
            return res.status(200).json({message: "patient fetched", ...patientShown})
        }else
           return res.status(400).json("didn't fetch patient!"); 
...
const updatePatient = async (req, res) => { 
    ...
    const updatedPatient = await updatePt(id, updatedPtData);
     if(updatedPatient){
           ...
           return res.status(400).json("didn't delete patient!"); 
} catch (error) {
     return res.status(400).json(error);
}
}
...
module.exports = { registerPatient, displaySinglePatient, displayPatients, updatePatient, deletePatient };